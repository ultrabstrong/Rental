    ...
    ILogger<RentalApplicationProcessor> logger) : IRentalApplicationProcessor
{
    ...
    public async Task ProcessAsync(RentalApplication rentalApplication)
    {
        _logger.LogDebug("Creating PDF for rental application");
        var pdf = await _pdfService.GenerateAsync(rentalApplication);

        _logger.LogDebug("Sending rental application email");
        using var pdfStream = new MemoryStream(pdf);
        await _emailService.SendEmailAsync(rentalApplication, pdfStream, CancellationToken.None);
    }
}
