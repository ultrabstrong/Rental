namespace ApartmentWeb.Middleware;
...
public class SecurityHeadersMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        {
            response.Headers.Append("Cache-Control", "no-cache, no-store, must-revalidate, private, max-age=0");
        ...
        response.Headers.Append("Content-Security-Policy", 
            ...
            "img-src 'self' data:; " +               // Added data: for inline SVGs and base64 images
            "font-src 'self' data:; " +              // Added data: for embedded fonts in CSS
            "connect-src 'self'; " +
            "form-action 'self'; " +
